...
struct conservative_vars {
    double rho;
    double rhou;
    double rhov;
    double rhoe;

    conservative_vars(double rho_, double rhou_, double rhov_, double rhoe_) {
        rho = rho_; rhou = rhou_; rhov = rhov_; rhoe = rhoe_;
    }
};


struct farfield_conditions {
    double mach;
    double angle=0;
    double T=1;
    double p=1.;
    ...
    farfield_conditions(double machi, double anglei=0, double Ti=1, double pi=1.) {
        mach = machi; angle = anglei; T = Ti; p = pi;
    }

    conservative_vars get_conservative(const gas& g) const {
        ...
        return conservative_vars(rho, rho*u, rho*v, rhoE);
    }
};
...
struct adim_scale {
    ...
    adim_scale() {}
    adim_scale(const farfield_conditions& var_far, const double& corde, const gas& g) {
